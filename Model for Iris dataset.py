...
def multinomial_logistic_regression(X_train, X_test, Y_train, Y_test, alpha, max_iter, method):
    ...
    models_list=[]
    models_name_list=['model_0','model_1','model_2']
    Y_train_list=[Y_train_0,Y_train_1,Y_train_2]
    Y_test_list = [Y_test_0,Y_test_1,Y_test_2]
    Y_hat = np.ones(Y_test.shape[0])
    for i in range(3):
        model = LogRegModel(X_train, X_test, Y_train_list[i], Y_test_list[i], alpha, max_iter, method)
        models_list.append(model)
        Y_hat_i = model["test_prediction"].reshape(-1,)
        for ind, elem in enumerate(Y_hat_i):
            if elem == i:
                Y_hat[ind] = elem
...
def bias_variance_decomp(X_train, y_train, X_test,y_test, alpha, max_iter, method, num_rounds = 5):
    ...
    for i in range(num_rounds):
        ...
        pred = multinomial_logistic_regression(X_boot, X_test, y_boot, y_test, alpha, max_iter, method)
        all_pred[i] = pred.reshape(y_test.shape[0],)
...
def computational_cost(X_train, X_test, Y_train, Y_test, alpha, max_iter):
    ...
    for method in methods:
        ...
        t_end = process_time()
        print('The', method ,'method for Iris has a computational cost of', (t_end - t_start)/5 ,'seconds.') 
       
def bias_vs_iterations(X_train, X_test, Y_train, Y_test, alpha):
    methods = ["SGD", "ISGD", "ADAM"]
    iterations = [5, 10, 25, 50, 100, 250]
    ...
    for ind, method in enumerate(methods):
        for max_iter in iterations:
            ...
            if (max_iter == 250):
                print('The', method ,'method for Iris has an asymptotic variance of', avg_var)
    ...
    plt.ylabel("Bias")
    sns.despine()
    plt.savefig("./Graphs/Iris_bias_iter.png")
    plt.show()
    plt.close()
    ...
    plt.yscale('log')
    sns.despine()
    plt.savefig("./Graphs/Iris_bias_iter_log.png")
    plt.show()
    plt.close()
...
def bias_vs_alpha(X_train, X_test, Y_train, Y_test, max_iter):
    ...
    plt.ylabel("Bias")
    sns.despine()
    plt.savefig("./Graphs/Iris_bias_alpha.png")
    plt.show()
    plt.close()
    ...
    plt.yscale('log')
    sns.despine()
    plt.savefig("./Graphs/Iris_bias_alpha_log.png")
    plt.show()
    plt.close()
...
def main():
    ...
    # Experiment 1
    computational_cost(X_train, X_test, Y_train, Y_test, alpha=0.01, max_iter=1000)

    # Experiment 2 and 4
    bias_vs_iterations(X_train, X_test, Y_train, Y_test, alpha=0.01)

    # Experiment 3
    bias_vs_alpha(X_train, X_test, Y_train, Y_test, max_iter=1000)
...
def citation():
    ...
    # Source for Dataset
    print("https://www.kaggle.com/datasets/arshid/iris-flower-dataset")
