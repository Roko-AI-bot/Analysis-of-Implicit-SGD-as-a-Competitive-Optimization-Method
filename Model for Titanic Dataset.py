...
def computational_cost(X_train, X_test, Y_train, Y_test, alpha, max_iter):
    ...
    for method in methods:
        ...
        t_end = process_time()
        print('The', method ,'method for Titanic has a computational cost of', (t_end - t_start)/5 ,'seconds.') 
...
def bias_vs_iterations(X_train, X_test, Y_train, Y_test, alpha):
    ...
    for ind, method in enumerate(methods):
        for max_iter in iterations:
            ...
            if (max_iter == 1000):
                print('The', method ,'method for Titanic has an asymptotic variance of', avg_var)
    ...
    plt.ylabel("Bias")
    sns.despine()
    plt.savefig("./Graphs/Titanic_bias_iter.png")
    plt.show()
    plt.close()
    ...
    plt.yscale('log')
    sns.despine()
    plt.savefig("./Graphs/Titanic_bias_iter_log.png")
    plt.show()
    plt.close()
...
def bias_vs_alpha(X_train, X_test, Y_train, Y_test, max_iter):
    ...
    plt.ylabel("Bias")
    sns.despine()
    plt.savefig("./Graphs/Titanic_bias_alpha.png")
    plt.show()
    plt.close()
    ...
    plt.yscale('log')
    sns.despine()
    plt.savefig("./Graphs/Titanic_bias_alpha_log.png")
    plt.show()
    plt.close()
...
def citation():
    # Source for Logistic Model code
